using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace GravityWebExt.Models
{
    /// <summary>
    /// Параметры - копия данных структуры NSPData (GravityCalc)
    /// </summary>
    public class NSPData
    {
        public int Id { get; set; }
        public double _devSx { get; set; }
        public double _devHx { get; set; }
        public double _devLx { get; set; }
        public double _devQx { get; set; }
        public double _devPx { get; set; }
        public double _devEPSILx { get; set; }
        public double _devFIx { get; set; }
        public double _devXpx { get; set; }
        public double _devYpx { get; set; }
        public double _devZpx { get; set; }
        public double _devMpx { get; set; }
        public double _devHpx { get; set; }
        public double _devDx { get; set; }
        public double _devEx { get; set; }
        public double _devMAx { get; set; }
        public double _devMMx { get; set; }

        public double _devSy { get; set; }
        public double _devHy { get; set; }
        public double _devLy { get; set; }
        public double _devQy { get; set; }
        public double _devPy { get; set; }
        public double _devEPSILy { get; set; }
        public double _devFIy { get; set; }
        public double _devXpy { get; set; }
        public double _devYpy { get; set; }
        public double _devZpy { get; set; }
        public double _devMpy { get; set; }
        public double _devHpy { get; set; }
        public double _devDy { get; set; }
        public double _devEy { get; set; }
        public double _devMAy { get; set; }
        public double _devMMy { get; set; }
        public double _devSz { get; set; }
        public double _devHz { get; set; }
        public double _devLz { get; set; }
        public double _devQz { get; set; }
        public double _devPz { get; set; }
        public double _devEPSILz { get; set; }
        public double _devFIz { get; set; }
        public double _devXpz { get; set; }
        public double _devYpz { get; set; }
        public double _devZpz { get; set; }
        public double _devMpz { get; set; }
        public double _devHpz { get; set; }
        public double _devDz { get; set; }
        public double _devEz { get; set; }
        public double _devMAz { get; set; }
        public double _devMMz { get; set; }
        public double _devSm { get; set; }
        public double _devHm { get; set; }
        public double _devLm { get; set; }
        public double _devQm { get; set; }
        public double _devPm { get; set; }
        public double _devEPSILm { get; set; }
        public double _devFIm { get; set; }
        public double _devXpm { get; set; }
        public double _devYpm { get; set; }
        public double _devZpm { get; set; }
        public double _devMpm { get; set; }
        public double _devHpm { get; set; }
        public double _devDm { get; set; }
        public double _devEm { get; set; }
        public double _devMAm { get; set; }
        public double _devMMm { get; set; }

        public double _devSx2 { get; set; }
        public double _devHx2 { get; set; }
        public double _devLx2 { get; set; }
        public double _devQx2 { get; set; }
        public double _devPx2 { get; set; }
        public double _devEPSILx2 { get; set; }
        public double _devFIx2 { get; set; }
        public double _devXpx2 { get; set; }
        public double _devYpx2 { get; set; }
        public double _devZpx2 { get; set; }
        public double _devMpx2 { get; set; }
        public double _devHpx2 { get; set; }
        public double _devDx2 { get; set; }
        public double _devEx2 { get; set; }
        public double _devMAx2 { get; set; }
        public double _devMMx2 { get; set; }

        public double _devSy2 { get; set; }
        public double _devHy2 { get; set; }
        public double _devLy2 { get; set; }
        public double _devQy2 { get; set; }
        public double _devPy2 { get; set; }
        public double _devEPSILy2 { get; set; }
        public double _devFIy2 { get; set; }
        public double _devXpy2 { get; set; }
        public double _devYpy2 { get; set; }
        public double _devZpy2 { get; set; }
        public double _devMpy2 { get; set; }
        public double _devHpy2 { get; set; }
        public double _devDy2 { get; set; }
        public double _devEy2 { get; set; }
        public double _devMAy2 { get; set; }
        public double _devMMy2 { get; set; }
        public double _devSz2 { get; set; }
        public double _devHz2 { get; set; }
        public double _devLz2 { get; set; }
        public double _devQz2 { get; set; }
        public double _devPz2 { get; set; }
        public double _devEPSILz2 { get; set; }
        public double _devFIz2 { get; set; }
        public double _devXpz2 { get; set; }
        public double _devYpz2 { get; set; }
        public double _devZpz2 { get; set; }
        public double _devMpz2 { get; set; }
        public double _devHpz2 { get; set; }
        public double _devDz2 { get; set; }
        public double _devEz2 { get; set; }
        public double _devMAz2 { get; set; }
        public double _devMMz2 { get; set; }

        public double _devSm2 { get; set; }
        public double _devHm2 { get; set; }
        public double _devLm2 { get; set; }
        public double _devQm2 { get; set; }
        public double _devPm2 { get; set; }
        public double _devEPSILm2 { get; set; }
        public double _devFIm2 { get; set; }
        public double _devXpm2 { get; set; }
        public double _devYpm2 { get; set; }
        public double _devZpm2 { get; set; }
        public double _devMpm2 { get; set; }
        public double _devHpm2 { get; set; }
        public double _devDm2 { get; set; }
        public double _devEm2 { get; set; }
        public double _devMAm2 { get; set; }
        public double _devMMm2 { get; set; }

        public GravityCalc.NSPData SetDataForCalculate()
        {
            GravityCalc.NSPData data = new();
            data.DefaultInit();
            //X1_________________________________________________________________________
            data._devSx = this._devSx;
            data._devHx = this._devHx;
            data._devLx = this._devLx;
            data._devQx = this._devQx;
            data._devPx = this._devPx;
            data._devEPSILx = this._devEPSILx;
            data._devFIx = this._devFIx;
            data._devXpx = this._devXpx;
            data._devYpx = this._devYpx;
            data._devZpx = this._devZpx;
            data._devMpx = this._devMpx;
            data._devHpx = this._devHpx;
            data._devDx = this._devDx;
            data._devEx = this._devEx;
            data._devMAx = this._devMAx;
            data._devMMx = this._devMMx;
            //Y1_________________________________________________________________________
            data._devSy = this._devSy;
            data._devHy = this._devHy;
            data._devLy = this._devLy;
            data._devQy = this._devQy;
            data._devPy = this._devPy;
            data._devEPSILy = this._devEPSILy;
            data._devFIy = this._devFIy;
            data._devXpy = this._devXpy;
            data._devYpy = this._devYpy;
            data._devZpy = this._devZpy;
            data._devMpy = this._devMpy;
            data._devHpy = this._devHpy;
            data._devDy = this._devDy;
            data._devEy = this._devEy;
            data._devMAy = this._devMAy;
            data._devMMy = this._devMMy;
            //Z1_________________________________________________________________________
            data._devSz = this._devSz;
            data._devHz = this._devHz;
            data._devLz = this._devLz;
            data._devQz = this._devQz;
            data._devPz = this._devPz;
            data._devEPSILz = this._devEPSILz;
            data._devFIz = this._devFIz;
            data._devXpz = this._devXpz;
            data._devYpz = this._devYpz;
            data._devZpz = this._devZpz;
            data._devMpz = this._devMpz;
            data._devHpz = this._devHpz;
            data._devDz = this._devDz;
            data._devEz = this._devEz;
            data._devMAz = this._devMAz;
            data._devMMz = this._devMMz;
            //M1_________________________________________________________________________
            data._devSm = this._devSm;
            data._devHm = this._devHm;
            data._devLm = this._devLm;
            data._devQm = this._devQm;
            data._devPm = this._devPm;
            data._devEPSILm = this._devEPSILm;
            data._devFIm = this._devFIm;
            data._devXpm = this._devXpm;
            data._devYpm = this._devYpm;
            data._devZpm = this._devZpm;
            data._devMpm = this._devMpm;
            data._devHpm = this._devHpm;
            data._devDm = this._devDm;
            data._devEm = this._devEm;
            data._devMAm = this._devMAm;
            data._devMMm = this._devMMm;
            //X2_________________________________________________________________________
            data._devSx2 = this._devSx2;
            data._devHx2 = this._devHx2;
            data._devLx2 = this._devLx2;
            data._devQx2 = this._devQx2;
            data._devPx2 = this._devPx2;
            data._devEPSILx2 = this._devEPSILx2;
            data._devFIx2 = this._devFIx2;
            data._devXpx2 = this._devXpx2;
            data._devYpx2 = this._devYpx2;
            data._devZpx2 = this._devZpx2;
            data._devMpx2 = this._devMpx2;
            data._devHpx2 = this._devHpx2;
            data._devDx2 = this._devDx2;
            data._devEx2 = this._devEx2;
            data._devMAx2 = this._devMAx2;
            data._devMMx2 = this._devMMx2;
            //Y2_________________________________________________________________________
            data._devSy2 = this._devSy2;
            data._devHy2 = this._devHy2;
            data._devLy2 = this._devLy2;
            data._devQy2 = this._devQy2;
            data._devPy2 = this._devPy2;
            data._devEPSILy2 = this._devEPSILy2;
            data._devFIy2 = this._devFIy2;
            data._devXpy2 = this._devXpy2;
            data._devYpy2 = this._devYpy2;
            data._devZpy2 = this._devZpy2;
            data._devMpy2 = this._devMpy2;
            data._devHpy2 = this._devHpy2;
            data._devDy2 = this._devDy2;
            data._devEy2 = this._devEy2;
            data._devMAy2 = this._devMAy2;
            data._devMMy2 = this._devMMy2;
            //Z2_________________________________________________________________________
            data._devSz2 = this._devSz2;
            data._devHz2 = this._devHz2;
            data._devLz2 = this._devLz2;
            data._devQz2 = this._devQz2;
            data._devPz2 = this._devPz2;
            data._devEPSILz2 = this._devEPSILz2;
            data._devFIz2 = this._devFIz2;
            data._devXpz2 = this._devXpz2;
            data._devYpz2 = this._devYpz2;
            data._devZpz2 = this._devZpz2;
            data._devMpz2 = this._devMpz2;
            data._devHpz2 = this._devHpz2;
            data._devDz2 = this._devDz2;
            data._devEz2 = this._devEz2;
            data._devMAz2 = this._devMAz2;
            data._devMMz2 = this._devMMz2;
            //M2_________________________________________________________________________
            data._devSm2 = this._devSm2;
            data._devHm2 = this._devHm2;
            data._devLm2 = this._devLm2;
            data._devQm2 = this._devQm2;
            data._devPm2 = this._devPm2;
            data._devEPSILm2 = this._devEPSILm2;
            data._devFIm2 = this._devFIm2;
            data._devXpm2 = this._devXpm2;
            data._devYpm2 = this._devYpm2;
            data._devZpm2 = this._devZpm2;
            data._devMpm2 = this._devMpm2;
            data._devHpm2 = this._devHpm2;
            data._devDm2 = this._devDm2;
            data._devEm2 = this._devEm2;
            data._devMAm2 = this._devMAm2;
            data._devMMm2 = this._devMMm2;


            return data;
        }
    }

}
